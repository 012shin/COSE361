...
class PerceptronModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        output = nn.DotProduct(x,self.get_weights())
        return output
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        second = nn.AddBias(nn.Linear(nn.ReLU(first),self.w2),self.b2)
        output = nn.AddBias(nn.Linear(nn.ReLU(second),self.w3),self.b3)
        return output
    ...
    def train(self, dataset):
        ...
        "*** YOUR CODE HERE ***"
        loss = float('inf')
        while loss >= 0.01: # loss must be < 0.02
            for x, y in dataset.iterate_once(self.batch_size):
                loss = self.get_loss(x, y)
                grads = nn.gradients(loss, self.params)

                loss = nn.as_scalar(loss) #typeError: '>=' not supported between instances of 'SquareLoss' and 'float'
...
class DigitClassificationModel(object):
    ...
    def __init__(self):
        ...
        self.batch_size = 100
        self.learning_rate = 0.05
        self.w1 = nn.Parameter(784,200) # hiddenlayer 200
    ...
    def run(self, x):
        ...
        output = nn.AddBias(nn.Linear(second_layer,self.w3),self.b3)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        # x
        self.xw = nn.Parameter(self.num_chars,300)
        self.xb = nn.Parameter(1,300)
        # hidden
        self.w2 = nn.Parameter(300,300)
        self.b2 = nn.Parameter(1,300)
        # output
        self.w3 = nn.Parameter(300,len(self.languages)) 
        self.b3 = nn.Parameter(1,len(self.languages))
        self.params = [self.w1, self.b1,self.xw,self.xb, self.w2, self.b2,self.w3,self.b3]
    ...
    def run(self, xs):
        ...
        for x in xs[1:]:
            part1 = nn.AddBias(nn.Linear(x,self.xw),self.xb)
            part2 = nn.AddBias(nn.Linear(h,self.w2),self.b2)
            h = nn.ReLU(nn.Add(part1,part2))
        output = nn.AddBias(nn.Linear(h,self.w3),self.b3)
        return output
